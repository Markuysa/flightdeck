...
<!--
  Vendored from https://github.com/Leonxlnx/taste-skill (MIT, © 2026 Leonxlnx),
  the skills/taste-skill/ variant. Copied rather than installed via `npx skills add`
  so it travels with the repository into cloud / autopilot sessions — same reasoning
  as skills/ponytail and skills/frontend-design.

  Body below is upstream verbatim; the only change is this note. The repo folder is
  `taste-skill`, so Claude Code registers it as `taste-skill` even though the
  frontmatter name is `design-taste-frontend` — wire it into agents as `taste-skill`.
  Scope (per §13): landing pages, portfolios, redesigns — NOT dashboards, data
  tables, or multi-step product UI.
